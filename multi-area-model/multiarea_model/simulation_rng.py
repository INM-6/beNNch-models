...
def connect(simulation,
    ...
    for target in target_area.populations:
        for source in source_area.populations:
            ...
            syn_weight = nest.random.normal(mean=W[target][source], std=W_sd[target][source])
            if target_area == source_area:
                if 'E' in source:
                    #syn_weight.update({'low': 0.})
                    syn_weight = nest.math.redraw(syn_weight, min=0., max=1000000000.)
                    mean_delay = network.params['delay_params']['delay_e']
                elif 'I' in source:
                    #syn_weight.update({'high': 0.})
                    syn_weight = nest.math.redraw(syn_weight, min=-10000000000., max=0.)
            ...
            syn_delay = nest.math.redraw(nest.random.normal(mean=mean_delay,
                                                            std=mean_delay * network.params['delay_params']['delay_rel']),
                                         min=simulation.params['dt'], max=100000000.)
